...
{ // Read a list of strings and frequencies from standard input,
  ...
  for (int i=0; i<count; i++) { // Read in the frequencies
    ...
    for(ptr=buff; *ptr==' '; ptr++);  // Read first word: shifts over until it finds the start of the object (i.e. ignoring tabbing in by spaces)
    Assert(*ptr == '"', "First char was not a quote mark.");
    for (ptr2=buff2,ptr++; *ptr!='"'; ptr++) //read everything inside the quotes: can be more than one char inside the quotes
      *ptr2++ = *ptr; //copy from ptr to ptr2, and then increment ptr2 to keep up with ptr
    *ptr2 = '\0'; // End of string
    for (ptr++; *ptr==' '; ptr++);//skip spaces to find the number
    Assert(isdigit(*ptr) != 0, "Must be a digit here.");
    freq = atoi(ptr);//converts everything at the current pointer and after into an integer since the ptr is starting at the current position in the array it will loop until it hits the null character '\0'
